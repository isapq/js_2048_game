...
function updateBoard() {
    ...
    for (const row of board)for (const cell of row){
        ...
        if (cell !== 0) td.classList.add(`field-cell--${cell}`);
        index++;
    }
}
 ...
 */ class Game {
   ...
   */ constructor(initialState){
    ...
    moveLeft(row) {
        ...
        for(let i = 0; i < arrayAxu.length - 1; i++)if (arrayAxu[i] === arrayAxu[i + 1]) {
            arrayAxu[i] *= 2;
        ...
        while(arrayAxu.length < 4)arrayAxu.push(0);
        return arrayAxu;
    }
    moveRight(row) {
        let arrayAxu = row.filter((number)=>number > 0);
        for(let i = arrayAxu.length - 1; i > 0; i--)if (arrayAxu[i] === arrayAxu[i - 1]) {
            arrayAxu[i] *= 2;
            this.score += arrayAxu[i];
            arrayAxu[i - 1] = 0;
            i--;
        ...
        arrayAxu = arrayAxu.filter((number)=>number > 0);
        while(arrayAxu.length < 4)arrayAxu.unshift(0);
        return arrayAxu;
    }
    moveUp(colum) {
   ...
   */ restart() {
    ...
    moveBoardLeft() {
        ...
        for(let i = 0; i < 4; i++)this.board[i] = this.moveLeft(this.board[i]);
        if (JSON.stringify(this.board) !== oldBoard) this.addRandomTile();
        this.checkGameStatus();
    }
    moveBoardRight() {
        const oldBoard = JSON.stringify(this.board);
        for(let i = 0; i < 4; i++)this.board[i] = this.moveRight(this.board[i]);
        if (JSON.stringify(this.board) !== oldBoard) this.addRandomTile();
        this.checkGameStatus();
    }
    moveBoardUp() {
    ...
    checkGameStatus() {
        ...
        for(let i = 0; i < 4; i++)for(let j = 0; j < 4; j++){
            ...
            if (this.board[j][i] === this.board[j + 1][i]) return;
        }
        // sem movimentos
        this.status = "lose";
    }
